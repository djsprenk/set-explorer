...
function energyGraph (container, songs, pois, setMetadata, index, scale) {
  // Dimensions / constant
  ...
  // Add a container
  const energyContainer = container.append('div').attr('class', 'energy-container')
  energyContainer.append('span').attr('class', 'graph-label').text('Energy')

  // Build the energy visualization
  const energyVisualization = energyContainer
    .append('svg')
  ...
  const gradient = energyVisualization.append('defs')
  ...
  energyVisualization.append('path')
    .datum(poisPoints)
    .attr('d', line)
  ...
  cuePoints.forEach((d, i) => {
    const x = getXPos(d, i)
    const y1 = graphHeight // getYPos(d, i)
    const y2 = 0 // yScale(graphMinBpm)

    energyVisualization.append('line')
      .attr('x1', x)
      .attr('y1', y1)
      .attr('x2', x)
      .attr('y2', y2)
