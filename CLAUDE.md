...
DJ analytics and visualization web app that renders interactive D3.js graphs of DJ sets (BPM, energy, playlists, etc.). Published at djsprenk.com. The repo is also known as `dj-analytics` on GitHub.
...
The pipeline runs in Python and outputs a single JS data file consumed by the frontend:
...
5. All sources → `scripts/data.py` (pandas joins/transforms) → `src/data/song-data.js`
...
- `src/scripts/index.js` — App entry point: initializes controls, loads set data, triggers rendering
- `src/scripts/controls.js` — Settings menu (BPM range, sort order, scaling, light/dark mode)
...
- `src/data/song-data.js` — **Generated file** — do not edit manually; produced by `make update-all`

Stylesheets follow SMACSS: `abstracts/` (vars/mixins), `components/` (per-graph styles), `pages/`, `theme/`.

### Build

- `webpack.config.dev.js` — Dev server config
- `webpack.config.prod.js` — Production build (splitChunks, HtmlWebpackPlugin, CopyPlugin)
- D3.js is pinned to v5.1.0; do not upgrade without verifying all graph code remains compatible.

### Code Style

- JavaScript: StandardJS (no semicolons, 2-space indent) — enforced via `npm run format`
